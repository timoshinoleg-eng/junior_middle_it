...
completed migration is a no-op unless --force is passed.
"""
...
def migrate(sqlite_path: str, dsn: str, force: bool = False) -> dict:
    ...
    # Ensure destination schema exists before the migration transaction.
    ...
    with psycopg.connect(dsn, autocommit=False) as target:
        with target.cursor() as cur:
            ...
            cur.execute(
                "SELECT value FROM growth_migration_meta WHERE key = 'sqlite_growth_v1'"
            )
            marker = cur.fetchone()
            ...
            if table_exists(source, "events"):
                rows = source.execute(
                    "SELECT ts, user_id, name, props FROM events ORDER BY id"
                ).fetchall()
                for row in rows:
                    try:
                    ...
                    cur.execute(
                        """
                        INSERT INTO growth_events (ts, user_id, name, props)
                        VALUES (COALESCE(%s, NOW()), %s, %s, %s)
                        """,
                        (row["ts"], row["user_id"], str(row["name"]), Jsonb(props)),
                    )
                    counts["events"] += 1
