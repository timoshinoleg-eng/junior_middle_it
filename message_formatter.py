...
This thin layer only owns acquisition/growth behaviour so it can evolve without
risking the formatting code that already has production coverage.
"""
...
def build_telegram_share_url(job: dict, bot_username: str = "") -> str:
    ...
    The shared object is the vacancy URL itself.  When a bot username is
    available, the accompanying text contains a deep-link campaign payload.
    ``cmd_start`` already records raw start payloads, so ``share_<job_hash>``
    gives us downstream acquisition attribution without an InlineQueryHandler
    or additional state.
    """
    ...
    if username:
        text_parts.append(
            f"Больше Junior/Middle remote IT: "
            f"https://t.me/{username}?start=share_{job_id}"
        )
...
class JobMessageFormatter(_BaseJobMessageFormatter):
    """Base formatter with a share button that works without inline mode."""
...
def get_formatter() -> JobMessageFormatter:
    """Return the growth-aware singleton formatter."""
    global _formatter
